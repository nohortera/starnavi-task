...
const App: FC = () => {
    const [selectedAppMode, setSelectedAppMode] = useState<IAppMode | undefined>({name: '', field: 0})
    const [appModes, isLoading, isError] = useFetch<IAppMode[]>('http://demo7919674.mockable.io', [])
    ...
    if (isError) {
        return (
            <h3 className='error'>Something went wrong. Check your internet connection.</h3>
        )
    }
      ...
      <div className='container'>
          <LoggerContext.Provider value={loggerContextValue}>
              <div>
                  ...
                  {selectedAppMode && <Field size={selectedAppMode.field} />}
              </div>
              <Logger logs={logs} />
